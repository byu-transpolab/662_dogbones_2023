...
```{r}
#| label: tbl-am-los
#| tbl-cap: Rebuild Model Results (AM Peak)

targets::tar_load(build_am_results)
build_am_results %>% format_los()
```

```{r}
#| label: tbl-pm-los
#| tbl-cap: Rebuild Model Results (PM Peak)

targets::tar_load(build_pm_results)
build_pm_results %>% format_los()
```
...
```{r}
#| label: fig-tt
#| fig-cap: Travel Time Comparison
...
build_am_traveltimes_formatted <- build_am_traveltimes_formatted %>% mutate(Model = "Rebuild", Label = paste0(Movement," (AM)"))
ex_pm_traveltimes_formatted <- ex_pm_traveltimes_formatted %>% mutate(Model = "Existing", Label = paste0(Movement," (PM)"))
build_pm_traveltimes_formatted <- build_pm_traveltimes_formatted %>% mutate(Model = "Rebuild", Label = paste0(Movement," (PM)"))
